...
def get_standoff_wp_poses(standoff_dist=0.1, tail_len=10, extra_off=0.01): #! Standoff pose of 10cm is kind of far
    """
    ...
    """
    offset = -standoff_dist * np.linspace(0, 1, tail_len, endpoint=False)[::-1] #! actually 9 cm standoff
    #offset = np.append(offset, [extra_off])
    #tail_len += 1 #! Destroys grasps data, you are not performing the original grasp
    #! This is the reason why scene replica collides with the table, 
    #! It also makes it squash objects in top down
...
def isaac_sort_and_filter_grasps(RT_obj, RT_gripper, RT_grasps, result_time, max_time, table_height: float):
    ...
    # Filter grasps with result_time >= 0
    # Filter grasps with result_time >= 0
    valid_mask = result_time >= 0
    ...
    # Filter grasps above table height + 0.02
    height_mask = z_values > (table_height + 0.02)
    RT_grasps_base = RT_world_frame[height_mask]
    result_time_filtered = result_time_valid[height_mask]
    indices_filtered = indices_valid[height_mask]
    ...
    RT_grasps_base = np.concatenate([RT_grasps_max] + sorted_grasps)
