...
def plan_grasp(
    ...
    for idx in range(n): 
        ...
        if plan[0]:
            ...
            #scene.remove_world_object(obj_name) #! Why remove the object? It is not added later for next iteration, Idk why but doesn't appear in panning scene
            waypoints = []
            ...
            if fraction >= 0.9:
                ...
                #scene.add_mesh(obj_name, p, obj_mesh_path) #! doesn't seem to work, commented it because of that
        else:
...
def grasp_with_rt(
    ...
    scene.remove_world_object(object_name) #! Remove after, this will allow you to lift object, but prevents the trajectory from squashing the object
...
if __name__ == "__main__":
    ...
    group.set_max_velocity_scaling_factor(0.5) #! 1 is wayyy too fast
    ...
    for obj_i, object_to_grasp in enumerate(object_order): # For every object in the scene
        ...
        setup_moveit_scene( #! Sets up boundaries and objects in moveit scene
            scene=scene, 
            table_height= table_height + 0.005, #! here would be the displacement for safety of colliding with table
        ...
        if direct_topdown or (grasp_num == -1 or (not trajectory_standoff) or (not trajectory_final)): 
            ...
            #if object_to_grasp in {"052_extra_large_clamp", "025_mug"}: #! exception objects, why? causes collisions with clamp
            #    g_width = -1
        ...
        # ------------------------ LIFTING OBJECT --------------------------#
        #! errors will arise from this gripper fully closed or fully open with new fingers
        if gripper.is_fully_closed() or gripper.is_fully_open(): 
            ...
            print("Lifting gripper to remove any collisions")#! The gripper should come up immediately if it fails a grasp (follow the reverse trajectory (alread computed))
            lift_arm_cartesian(group, RT_gripper) #! Won't lift if it is inside a collider in planning scene
        else:
